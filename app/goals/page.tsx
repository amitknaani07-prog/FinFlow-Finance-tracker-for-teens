...
export default function GoalsPage() {
  ...
  const handleContribute = async (e: React.FormEvent, goal: any) => {
    ...
    if (!error) {
      if (completed && !goal.completed) {
        ...
        if (res.bonusAwarded) alert(res.message); // simple notification
      }
  ...
  return (
    <div className="p-4 pt-8 md:p-8 max-w-2xl mx-auto space-y-8 pb-32">
      ...
      {isCreating && (
        <div className="bg-surface p-6 rounded-3xl border border-white/5 animate-in fade-in slide-in-from-top-4">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Target className="w-5 h-5 text-accent" /> Create a Goal</h3>
      ...
      <div className="space-y-6">
        {goals.map(goal => {
          ...
          return (
            <div key={goal.id} className="bg-gradient-to-br from-surface to-[#111318] p-6 rounded-3xl border border-white/5 relative overflow-hidden shadow-xl">
              {celebratingId === goal.id && (
                <div className="absolute inset-0 bg-accent/20 flex flex-col items-center justify-center z-10 animate-in fade-in duration-300">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                {isComplete && <div className="text-accent text-xs font-bold uppercase tracking-wider bg-accent/10 px-2 py-1 rounded-md">Completed</div>}
              </div>
              ...
              {!isComplete && (
                <div className="mt-6 border-t border-white/5 pt-4">
                  {contributeGoalId === goal.id ? (
                    <form onSubmit={(e) => handleContribute(e, goal)} className="flex gap-2 animate-in slide-in-from-top-2">
                      <input 
